...
const validateVariable = (variable, strict = false) => {
	let replaced
	let es3
	let es5
	let unicode
	...
	es5 = !regexIdentifierES5.test(
		// Only Unicode escapes are allowed in ES5 identifiers.
		variable.replace(/\\u([a-fA-F0-9]{4})/g, function ($0, $1) {
			return String.fromCodePoint(parseInt($1, 16))
		})
	)

	invalidIdentifier = !regexIdentifier.test(replaced)
	reserved = regexES6ReservedWord.test(replaced)

	if (invalidIdentifier || reserved) {
		result = {
			valid: false
		}
		...
		errors = Object.keys(errors).reduce((previousValue, currentValue) => {
			if (errors[currentValue]) {
				return {
					...previousValue,
					[currentValue]: errors[currentValue]
				}
			}
		...
		es3 = regexES3ReservedWordExclusive.test(replaced)

		if (!regexIdentifier.test(replaced)) {
			es5 = true
			es3 = false
		}

		unicode = !regexIdentifierUnicode5.test(replaced)
		immutable = regexImmutableProps.test(replaced)
		zeroWidth = regexZeroWidth.test(replaced)
		...
		warnings = Object.keys(warnings).reduce(
			(previousValue, currentValue) => {
				if (warnings[currentValue]) {
					return {
						...previousValue,
...
const isValidVariable = (variable, strict) => {
	return validateVariable(variable, strict).valid
}
